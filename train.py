...
def main():
    ...
    if opt.model['use_resnet'] == 1:
        model = BaselineNet7(opt.model['out_c'], opt.model['resnet_layers'], opt.model['train_res4'])
        logger.info('choosing BaselineNet7(resnet based) model')
    else:
    ...
    if opt.model['use_resnet'] == 1:
        train_set = HCCDataset6('{:s}/train{:s}.h5'.format(opt.train['data_dir'], fold_num), data_transform, opt)
        test_set = HCCDataset6('{:s}/test{:s}.h5'.format(opt.train['data_dir'], fold_num), data_transform, opt)
    else:
        train_set = HCCDataset6('{:s}/train{:s}.h5'.format(opt.train['data_dir'], fold_num), data_transform)
        test_set = HCCDataset6('{:s}/test{:s}.h5'.format(opt.train['data_dir'], fold_num), data_transform)
...
def train(opt, train_set, model, criterion, optimizer, epoch):
    ...
    for i in range(0, len(idx_list), opt.train['batch_size']):
        ...
        for k in range(N):
            idx = idx_list[i + k]
            ct_data,label,scale_labels = train_set[idx]

            input_data = ct_data

            output, scales = model(input_data.cuda())
            loss += criterion(output, label.cuda())
            loss_scales += criterion(scales, scale_labels.squeeze(0).cuda())
...
def test(opt, test_set, model, criterion, epoch):
    ...
    for i in range(len(test_set)):
        ct_data, label,_ = test_set[i]

        with torch.no_grad():
            input_data = ct_data
            output,_ = model(input_data.cuda())
            loss = criterion(output, label.cuda())
